...
def update(val):
    ...
    for i, (full_data, base_title) in enumerate(datasets):
        ...
        # 4. Ausgabe-Text für die "maximale b-Bevorzugung"
        if best_b_sample:
            ...
            effektiver_anteil = max_b_score * 100
        else:
            formatted_text = "Keine Daten"
            effektiver_anteil = 0.0
            
        info_texts[i].set_text(
            f"Maximale 'b'-Ausbeute\n({effektiver_anteil:.1f}% 'b' bei n={current_size}):\n{formatted_text}"
        )
...
def save_report(event):
    ...
    print(f"[ERFOLG] Belastungsbericht gespeichert unter: {ausgabe_name}")
...
plt.suptitle("Quiz-im-Vierteltakt: Näherung maximaler 'b'-Häufungen", fontsize=14, fontweight="bold", y=0.98)
plt.show()
